...
report = Report.fullReports(eon)
Report.printReports(eon, report)
Report.saveReports(eon, report, 'results/')
Figure.save(eon, 'results/')

# Simulating
print('\nOriginal graph simulation')
Simulation.simulateRandomDemands(eon, random_state=10)
print(Report.reportsFromDemands(eon))
...
for i in range(len(possible_eons)):
    ...
    print(Report.reportsFromDemands(possible_eons[i]))